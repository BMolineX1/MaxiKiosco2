...
namespace CapaDatos
{
    public class CD_CuentaCorriente
    {
        ...
        public bool AumentarImpagosGlobal(decimal porcentaje, int? clienteId, out string msg)
        {
            msg = "";
            using var cn = new MySqlConnection(Conexion.cadena);
            using var cmd = new MySqlCommand("SP_CC_AumentarImpagosGlobal", cn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("p_porcentaje", porcentaje);
            if (clienteId.HasValue) cmd.Parameters.AddWithValue("p_cliente_id", clienteId.Value);
            else cmd.Parameters.AddWithValue("p_cliente_id", DBNull.Value);
            try { cn.Open(); cmd.ExecuteNonQuery(); return true; }
        ...
        public bool PagarPorItems(int clienteId, int ventaId, int productoId, int cantidad, int usuarioId, out string msg)
        {
            msg = "";
            using var cn = new MySqlConnection(Conexion.cadena);
            using var cmd = new MySqlCommand("SP_CC_PagarPorItems", cn);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("p_cliente_id", clienteId);
            cmd.Parameters.AddWithValue("p_venta_id", ventaId);
            cmd.Parameters.AddWithValue("p_producto_id", productoId);
        ...
        public bool RevaluarPendientes(int idCliente, int idUsuario, out string mensaje)
        {
            ...
            {
                using (var cn = new MySqlConnection(Conexion.cadena))
                using (var cmd = new MySqlCommand("SP_CC_RevaluarPendientes", cn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;

                    // 🔴 ANTES lo tenías como p_idcliente / p_idusuario
                    // ✅ TIENE que coincidir con la firma del SP:
                    cmd.Parameters.AddWithValue("p_ClienteId", idCliente);
                    cmd.Parameters.AddWithValue("p_UsuarioId", idUsuario);
        ...
        // Método existente que usa SP_CC_PagarProducto con parámetros de salida
        public bool CCPagarProducto(int clienteId, int productoId, int cantidad, int usuarioId, out string mensaje)
        {
            mensaje = "";
            using (var cn = new MySqlConnection(Conexion.cadena))
            using (var cmd = new MySqlCommand("SP_CC_PagarProducto", cn))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("p_ClienteId", clienteId);
                cmd.Parameters.AddWithValue("p_ProductoId", productoId);
                cmd.Parameters.AddWithValue("p_Cantidad", cantidad);
                cmd.Parameters.AddWithValue("p_UsuarioId", usuarioId);
        ...
        SELECT 
            ...
            p.codigo           AS Codigo,
            p.nombre           AS Nombre,
            d.cantidad_pend    AS Pendiente,        -- unidades
            d.precio_unitario  AS PrecioUnitario,   -- precio por unidad (ya revaluado)
            d.monto_pendiente  AS MontoPendiente,   -- total pendiente
            0                  AS Pagar,
            0                  AS SubTotal
        FROM cc_detalle d
        INNER JOIN producto p ON p.id = d.producto_id
        WHERE d.cliente_id = @cli
          AND d.cantidad_pend > 0
        ORDER BY p.nombre";
        ...
        // 🔹 NUEVO: listado específico para el modal, con alias que matchean el grid
        public DataTable ListarPendientesPorCliente(int clienteId)
        {
            var dt = new DataTable();
            const string sql = @"
        SELECT 
            ...
            p.codigo          AS Codigo,
            p.nombre          AS Nombre,
            d.cantidad_pend   AS Pendiente,
            p.precioventa     AS PrecioVigente,
            0                 AS Pagar,
            0                 AS SubTotal
        FROM cc_detalle d
        ...
        public DataTable ListarCuentas()
        {
            ...
            const string sql = @"
                SELECT 
                    cli.id AS Id,
                    COALESCE(CONCAT(cli.apellido,' ',cli.nombre)) AS Cliente,
                    ccc.saldo AS Saldo,
