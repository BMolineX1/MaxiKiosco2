...
namespace CapaDatos
{
    public class CD_Proveedor
    {
        ...
        public decimal ObtenerPorcentajeAumento(int idProveedor)
        {
            decimal porcentaje = 0m;
            string query = "SELECT porcentaje_aumento FROM proveedor WHERE id = @id";
            ...
            using (var conexion = new MySqlConnection(Conexion.cadena))
            {
                using (var cmd = new MySqlCommand(query, conexion))
                {
                    ...
                    try
                    {
                        ...
                        {
                            decimal.TryParse(resultado.ToString(), out porcentaje);
                        }
                    }
                    catch { porcentaje = 0m; }
                }
