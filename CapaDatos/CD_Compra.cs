    ...
    public class CD_Compra
    {
        ...
        public bool Registrar(Compra obj, DataTable detalleCompra, out string mensaje)
        {
            ...
            var items = detalleCompra.AsEnumerable().Select(r => new
            {
                ...
                monto_iva = Convert.ToDecimal(r["monto_iva"]),  // 🎯 NUEVOS CAMPOS DEL DATATABLE
                precio_bruto = Convert.ToDecimal(r["precio_bruto"])
            ...
            using (var cmd = new MySqlCommand("SP_RegistrarCompra", cn))
            {
                ...
                cmd.Parameters.AddWithValue("p_montototal", obj.montototal);// Total Bruto de la compra
                 // Usa el nombre que definimos en el SP de MySQL.
                cmd.Parameters.AddWithValue("p_monto_iva_total", obj.monto_iva_total);
        ...
        public List<Compra> ListarCabeceras(int max = 300)
        {
            var lista = new List<Compra>();

            string sql = @"
        SELECT 
            c.id_compra          AS id,            -- ↩ PK real
            c.numerodocumento,
            c.tipodocumento,
            c.fecharegistro,
            pr.id                AS proveedor_id,  -- ↩ coincide con Proveedor.id
            pr.cuit,
            pr.razonsocial
        FROM compra c
        LEFT JOIN proveedor pr ON pr.id = c.proveedor_id
        ORDER BY c.fecharegistro DESC, c.id_compra DESC
        LIMIT @max;";
            ...
            using (var cmd = new MySqlCommand(sql, cn))
            {
                ...
                using (var dr = cmd.ExecuteReader(CommandBehavior.CloseConnection))
                {
                    ...
                    int i_numero = dr.GetOrdinal("numerodocumento");
                    int i_tipo = dr.GetOrdinal("tipodocumento");
                    int i_fecha = dr.GetOrdinal("fecharegistro");
                    int i_proveedor_id = dr.GetOrdinal("proveedor_id");
                    int i_cuit = dr.GetOrdinal("cuit");
                    int i_razonsocial = dr.GetOrdinal("razonsocial");
                    ...
                    while (dr.Read())
                    {
                        ...
                        {
                            id = dr.GetInt32(i_id),
                            numerodocumento = dr.IsDBNull(i_numero) ? "" : dr.GetString(i_numero),
                            tipodocumento = dr.IsDBNull(i_tipo) ? "" : dr.GetString(i_tipo),
                            fecharegistro = dr.GetDateTime(i_fecha),
                            oproveedor = new Proveedor
                            {
                                id = dr.IsDBNull(i_proveedor_id) ? 0 : dr.GetInt32(i_proveedor_id),
                                cuit = dr.IsDBNull(i_cuit) ? "" : dr.GetString(i_cuit),
                                razonsocial = dr.IsDBNull(i_razonsocial) ? "" : dr.GetString(i_razonsocial)
                            }
                        };
                        lista.Add(comp);
                    }
