...
namespace MaxiKiosco
{
    public partial class frmCompras : Form
    {
        ...
        private void CargarDatosProducto(Producto p)
        {
            ...
            // Precios (respeta lo que venga de BD; si PV <= PC y hay % categoria, sugiere)
            decimal precioCompra = p.preciocompra;
            decimal precioVenta = p.precioventa;
            decimal porcentajeAumento = p.ocategoria?.porcentaje_aumento ?? 0m;

            if (porcentajeAumento > 0 && precioVenta <= precioCompra)
            {
                decimal multiplicador = 1 + (porcentajeAumento / 100m);
                precioVenta = precioCompra * multiplicador;
            }

            txtpreciocompraproducto.Text = precioCompra.ToString("0.00");
            txtprecioventaproducto.Text = precioVenta.ToString("0.00");

            // IVA del producto (21% de PRECIO VENTA)
            txtivaProducto.Text = (precioVenta * IVA_PERCENTAJE).ToString("0.00");
        ...
        private void calcularTotal()
        {
            ...
            // Total a pagar (solo suma de compras). Si querés incluir IVA a nivel total, descomentar:
            // total = total + (total * IVA_PERCENTAJE);
        ...
        private void btnregistrarcompra_Click(object sender, EventArgs e)
        {
            ...
            decimal montoTotalCompra;
            if (!decimal.TryParse(txttotalpagar.Text, NumberStyles.Any, CultureInfo.CurrentCulture, out montoTotalCompra) &&
                !decimal.TryParse(txttotalpagar.Text, NumberStyles.Any, CultureInfo.InvariantCulture, out montoTotalCompra))
            {
            ...
            {
                MessageBox.Show("El monto total debe ser mayor a cero.", "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                return;
            }
