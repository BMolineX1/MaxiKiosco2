...
namespace MaxiKiosco
{
    public partial class Inicio : Form
    {
        ...
        {
            // ==============================================
            // 1. 🛡️ GUARDIÁN DE DATOS (Intercepción de Cierre)
            // ==============================================
            if (FormularioActivo != null)
            {
                // 1.1. Verificamos si el formulario activo es el de Compras
                if (FormularioActivo is frmCompras frmCompraActivo)
                {
                    // 1.2. Usamos el método guardián del formulario de Compras
                    if (!frmCompraActivo.PuedeCambiarDeVentana())
                    {
                        // Si el usuario presiona 'No', NO CAMBIAMOS de menú,
                        // NO cerramos el formulario actual y salimos del método.
                        return;
                    }
                    // Si el usuario presiona 'Sí', o no hay ítems, continuamos.
                }

                // Cierra el formulario activo (si no se canceló en el paso anterior)
        ...
        private void Inicio_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (ap != null)
            {
                ...
                using (var frm = new frmCajasMaestroDetalle())
                {
                    frm.ShowDialog(this);
                }
        ...
        private void AjustePrecios_Click(object sender, EventArgs e)
        {
            AbrirFormulario((IconMenuItem)sender, new frmSubaPrecio());
